...
void addClient_window(people::Manager &manager) {
    ...
    manager.add_client({email, name, phone, deal_product}, true, std::cout);
    std::cout << "Ready? - 1" << '\n';
    int number;
    std::cin >> number;
    assert(number == 1);
...
void cilents_window(people::Manager &manager) {
    ...
    int number;
    std::cin >> number;
    if (number == 2) { addClient_window(manager); }
    if (number == 3) { general_window(manager); }
    assert(false);
}
...
void login_window() {
    ...
    if (!is_exists || !is_password) { login_window(); }
...
void registration_window() {
    ...
    std::cin >> name >> phone >> email >> password;
    people::Manager manager(name, phone, email, password);
    people::add_manager(manager, std::cout);
    std::cout << "Welcome" << '\n';
    general_window(manager);
}
